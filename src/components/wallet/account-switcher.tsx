        ...
        {isOpen && (
          ...
          >
            ...
            {connectedWallets.map(([id, w]) => (
              <button key={id} role="option" aria-selected={activeWalletId === id} onClick={() => { switchWallet(id); setIsOpen(false) }} className={cn("w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-all", activeWalletId === id ? "glass-strong text-foreground" : "text-muted-foreground hover:text-foreground hover:glass-whisper")}>
