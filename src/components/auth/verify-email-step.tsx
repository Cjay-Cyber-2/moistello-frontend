...
export function VerifyEmailStep({ onVerified, onBack }: VerifyEmailStepProps) {
    ...
    return (
      <>
        ...
        <p className="text-sm text-muted-foreground -mt-2 mb-2">
          We sent a code to <strong className="text-foreground">{emailVerification.email}</strong>
        </p>
        ...
        <Button variant="primary" size="lg" className="w-full" onClick={handleVerify}
          isLoading={isVerifying} disabled={code.length !== 6} leftIcon={<Shield className="h-4 w-4" />}>
          Verify Code
        </Button>
  ...
  return (
    ...
    <>
      ...
      <Input label="Email address" type="email" placeholder="you@example.com" value={emailInput}
        onChange={(e) => { setEmailInput(e.target.value); setError("") }}
        onKeyDown={(e) => e.key === "Enter" && handleSendCode()}
        leftIcon={<Mail className="h-4 w-4" />} error={error} />
      <Button variant="primary" size="lg" className="w-full" onClick={handleSendCode}
        isLoading={isSendingCode} disabled={!emailInput.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)}
