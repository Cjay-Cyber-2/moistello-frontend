...
export default function CreateProposalPage() {
  ...
  async function submit() {
    ...
    setError("")
    try {
      const proposal = await createProposal.mutateAsync({ title, description, executionPayload })
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <form className="mt-10 space-y-8" onSubmit={(event) => { event.preventDefault(); void submit() }}>
        ...
        <Button type="submit" isLoading={createProposal.isPending} leftIcon={<Send className="h-4 w-4" />} rightIcon={<FileCode2 className="h-4 w-4" />}>Submit proposal</Button>
      </form>
    </div>
  )
}
