...
import {
  ArrowUpRight, ArrowDownRight, ArrowDownLeft, Wallet as WalletIcon,
  Settings, Clock, ArrowRight, ListOrdered, BookCopy, ExternalLink,
  QrCode, Copy, Check, RefreshCw, Send,
} from "lucide-react"
...
export default function WalletPage() {
  ...
  // ── Real balance with polling via proxy ──────────────────────────────────
  const { balance, isLoading: balanceLoading, error: balanceError, refresh: refreshBalance, lastUpdatedAt } =
  ...
  useEffect(() => {
    async function loadActivity() {
      setTxLoading(true)
      try {
        const [contribRes, payoutRes] = await Promise.allSettled([
        ...
        console.error("[wallet] Failed to load activity:", e)
      } finally {
        setTxLoading(false)
      }
    }
  ...
  return (
    <div className="space-y-8">
      ...
      {showReceive && wallet && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-2xl bg-[rgb(var(--background))] border border-white/15 p-6 space-y-5"
      ...
      <div>
        ...
        {balanceError && (
          <div className="mb-3 px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-xs text-red-400">
            {balanceError} — showing cached values
          </div>
        )}
          ...
          <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-aurora-violet/10 to-aurora-indigo/5 border border-aurora-violet/15 p-6">
            ...
            <p className="text-3xl font-bold font-heading text-foreground">
              {balanceLoading && !balance
                ? <span className="inline-block w-24 h-8 bg-white/10 rounded animate-pulse align-middle" />
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {/* Send — links to withdraw page (transfer out) */}
